...
if __name__ == '__main__':
    ...
    '''Randomly select 6 datasets as the test data, the rest as the train data'''
    ...
    '''Remove the inf value in the data'''
    count = 0
    ...
    fpr, tpr, thresholds = roc_curve(test_label, y_score[:, 1], pos_label=1)
    roc_auc = auc(fpr, tpr)
    plt.figure()
    lw = 2
    plt.plot(fpr, tpr, color='darkorange', lw=lw, label='ROC curve (area = %0.2f)' % roc_auc)
