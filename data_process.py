...
if __name__ == '__main__':
    # GSE = pd.read_csv('./dataset/Strep Data/data.csv', index_col=0)
    # print('GSE_ori_len: ', GSE.shape)
    # GSE_label = pd.read_csv('./dataset/Strep Data/label.csv', index_col=0)
    # print('label_len: ', GSE_label.shape)
    # GSE, GSE_label = length_eqaul(GSE, GSE_label)
    # GSE = drop('./c2.cp.kegg.v7.2.symbols.csv', GSE)
    # print('GSE_new_len: ', GSE.shape)
    # print('label_new_len: ', GSE_label.shape)
    #
    # print('numbers of 0: ', np.sum(GSE_label == 0))
    # print('numbers of 1: ', np.sum(GSE_label == 1))
    # print('numbers of 2: ', np.sum(GSE_label == 2))
    #
    # start_time = time.time()
    # pair_index_exact_expressed = fisher_exact_test(GSE, GSE_label)
    # print("---Final time cost: %s seconds ---" % (time.time() - start_time))
    # print(pair_index_exact_expressed)
    # file = open('./iPAGE_result/RSV/pair_index_exact_expressed_all.txt', 'w')
    # file.write(str(pair_index_exact_expressed))
    # file.close()
